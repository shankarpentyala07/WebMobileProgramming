...
app.get('/get', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection('mongo_books').find().toArray(function(err, result){
            if(err)
            {
...
var deleteDocument = function (db, id, callback) {
    db.collection('mongo_books').deleteOne( {_id : new mongo.ObjectId(id)}, function(err, result) {
        if (err) {
...
var updateDocument = function (db, id, book, res, callback) {
    console.log("Id "+id);
    db.collection('mongo_books').updateOne( {_id : new mongo.ObjectId(id)}, book, function(err, result) {
        if (err) {
...
var insertDocument = function(db, data, callback) {
    db.collection('mongo_books').insertOne( data, function(err, result) {
        if(err)
        {
